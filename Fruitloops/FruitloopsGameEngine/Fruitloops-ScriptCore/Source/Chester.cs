...
\author  Ming Sheng
\date    March 14, 2025
\brief  Implements the Chester class, a behavior component for managing an interactive
        treasure chest entity that adds an item to the inventory when clicked.
...
{
    /**
     * @brief Manages a treasure chest entity with button-like behavior.
     * 
     * @note Handles chest interaction, adding an item to the inventory when clicked
     * by the player when adjacent, playing a sound, and updating the chest's texture.
     * Inherits from BehaviourComponent.
     */
    public class Chester : BehaviourComponent
    {
        ...
        /**
         * @brief Default constructor for Chester.
         */
        public Chester() { }

        /**
         * @brief Constructs a Chester instance for a specific entity.
         * 
         ...
         */
        public Chester(Entity entity)
        {
            Debug.LogInfo("[Chester] Creating for entity {0}", entity.ID);

            if (!entity.HasLocalComponent<ButtonComponent>())
            {
                Debug.LogInfo("[Chester] Adding ButtonComponent to entity {0}", entity.ID);
        ...
        /**
         * @brief Creates a new Chester behavior instance for an entity.
         * 
         * @param entity The Entity to associate with the chest behavior.
         * @return A new Chester instance, or null if an exception occurs.
        ...
        public static BehaviourComponent CreateBehaviour(Entity entity)
        {
            try
            {
                Debug.LogInfo("[Chester] [CreateBehaviour] Starting for entity {0}", entity.ID);
                return new Chester(entity);
            }
            catch (Exception ex)
            {
                Debug.LogError("[Chester] [CreateBehaviour] Exception: {0}", ex.Message);
                return null;
            }
        }

        /**
         * @brief Initializes the Chester behavior for an entity.
         * 
        ...
        {
            Debug.LogInfo("[Chester] Initializing for entity {0}", entity.ID);
        }
         ...
         * @note Handles hover detection, click processing to open the chest, item addition,
         * energy deduction, texture update, and sound playback when adjacent to the player.
         */
        public static void Update(Entity entity, float deltaTime)
        {
            var button = GameLogicSystem.Instance.GetBehaviour<Chester>(entity);
            if (button == null || button._buttonComponent == null)
            {
                Debug.LogWarning("[Chester] ButtonComponent is null for entity {0}.", entity.ID);
                return;
            }
            ...
            try
            {
                ...
                ulong chestID = InternalCalls.Entity_GetEntityByName("TreasureChest");
                ...
                if (Entity_IsAdjacent(playerID, chestID, tileSize))
                {
                    if (Input.IsMouseButtonDown(0) && button._buttonComponent.IsHovered && !button._clickProcessed)
                    {
                        ...
                        {
                            Debug.LogInfo("[Chester] Button clicked on entity {0}", entity.ID);
                            ...
                            InternalCalls.SetEntityChestTexture("cheese_treasure_chest");
            ...
            {
                Debug.LogError("[Chester] Error during update: {0}", ex.Message);
            }
        }

        /**
         * @brief Cleans up the Chester behavior.
         * 
         * @param entity The Entity associated with the chest behavior.
        ...
        public static void End(Entity entity)
        {
            ...
            Debug.LogInfo("[Chester] Cleanup complete for entity {0}.", entity.ID);
        }
        ...
        private static bool Input_IsMouseOverEntity(ulong entityID)
        {
            ...
            {
                Debug.LogError("[Chester] Entity {0} missing Transform: {1}", entityID, ex.Message);
                return false;
            }
