/******************************************************************************
/*!
\file  ChesterChicken.cs
\Proj name  Shroomy Doomy
\author  
\date    March 14, 2025
\brief  Implements the ChesterChicken class, a behavior component for managing an interactive
        chicken treasure chest entity that adds "chicken" to the inventory when clicked.
...
{
    /**
     * @brief Manages a chicken treasure chest entity with button-like behavior.
     * 
     * @note Handles chest interaction, adding "chicken" to the inventory when clicked
     * by the player when adjacent, updating the chest's texture, and playing a sound.
     * Inherits from BehaviourComponent.
     */
    public class ChesterChicken : BehaviourComponent
    {
        private ButtonComponent _buttonComponent;
        private bool _clickProcessed = false;

        private static bool isopenchest = false;
        public static bool IsOpenChest => isopenchest;

        /**
         * @brief Default constructor for ChesterChicken.
         */
        public ChesterChicken() { }

        /**
         * @brief Constructs a ChesterChicken instance for a specific entity.
         * 
         ...
         */
        public ChesterChicken(Entity entity)
        {
            Debug.LogInfo("[ChesterChicken] Creating for entity {0}", entity.ID);

            if (!entity.HasLocalComponent<ButtonComponent>())
            {
                Debug.LogInfo("[ChesterChicken] Adding ButtonComponent to entity {0}", entity.ID);
        ...
        /**
         * @brief Creates a new ChesterChicken behavior instance for an entity.
         * 
         * @param entity The Entity to associate with the chest behavior.
         * @return A new ChesterChicken instance, or null if an exception occurs.
        ...
        public static BehaviourComponent CreateBehaviour(Entity entity)
        {
            try
            {
                Debug.LogInfo("[ChesterChicken] [CreateBehaviour] Starting for entity {0}", entity.ID);
                return new ChesterChicken(entity);
            }
            catch (Exception ex)
            {
                Debug.LogError("[ChesterChicken] [CreateBehaviour] Exception: {0}", ex.Message);
                return null;
            }
        }

        /**
         * @brief Initializes the ChesterChicken behavior for an entity.
         * 
        ...
        {
            Debug.LogInfo("[ChesterChicken] Initializing for entity {0}", entity.ID);
            isopenchest = false;
        }
         ...
         * @note Handles hover detection, click processing to open the chest, adds "chicken" to
        ...
        {
            var button = GameLogicSystem.Instance.GetBehaviour<ChesterChicken>(entity);
            if (button == null || button._buttonComponent == null)
            {
                Debug.LogWarning("[ChesterChicken] ButtonComponent is null for entity {0}.", entity.ID);
                return;
            }
            ...
            try
            {
                ...
                ulong chestID = InternalCalls.Entity_GetEntityByName("ChesterChicken");
                ...
                if (Entity_IsAdjacent(playerID, chestID, tileSize))
                {
                    if (Input.IsMouseButtonDown(0) && button._buttonComponent.IsHovered && !button._clickProcessed)
                    {
                        ...
                        {
                            Debug.LogInfo("[ChesterChicken] Button clicked on entity {0}", entity.ID);
                            InventorySystem.Instance?.AddItem("chicken"); // Call the AddItem function
                            ...
                            InternalCalls.SetEntityTextureByID(entity.ID, "chicken_treasure_chest");
                            ...
                            InternalCalls.SetChestOverlayTrigger("ChesterChickenTime");
            ...
            {
                Debug.LogError("[ChesterChicken] Error during update: {0}", ex.Message);
            }
        }

        /**
         * @brief Cleans up the ChesterChicken behavior.
         * 
         * @param entity The Entity associated with the chest behavior.
        ...
        public static void End(Entity entity)
        {
            ...
            Debug.LogInfo("[ChesterChicken] Cleanup complete for entity {0}.", entity.ID);
        }
        ...
        private static bool Input_IsMouseOverEntity(ulong entityID)
        {
            ...
            {
                Debug.LogError("[ChesterChicken] Entity {0} missing Transform: {1}", entityID, ex.Message);
                return false;
            }
