...
\author  Ming Sheng
\date    Feb 03, 2024
\brief  This file implements the DisableHP class, a behaviour component that
...
namespace FruitLoops
{
     ...
     *
     * The DisableHP listens for mouse input over its associated entity's button.
     * When clicked, it calls the InventoryManager's AddItem method to add an item ("tomato").
     */
    public class DisableHP : BehaviourComponent
    {
        ...
        /**
         * @brief Constructs a new DisableHP behaviour for the specified entity.
         *
         ...
         */
        public DisableHP(Entity entity)
        {
            Debug.LogInfo("[DisableHP] Creating for entity {0}", entity.ID);

            if (!entity.HasLocalComponent<ButtonComponent>())
            {
                Debug.LogInfo("[DisableHP] Adding ButtonComponent to entity {0}", entity.ID);
        ...
        /**
         * @brief Factory method to create an DisableHP behaviour.
         *
         ...
         * @return A new DisableHP instance as a BehaviourComponent, or null if an exception occurs.
         */
        public static BehaviourComponent CreateBehaviour(Entity entity)
        {
            try
            {
                Debug.LogInfo("[DisableHP] [CreateBehaviour] Starting for entity {0}", entity.ID);
                return new DisableHP(entity);
            }
            catch (Exception ex)
            {
                Debug.LogError("[DisableHP] [CreateBehaviour] Exception: {0}", ex.Message);
                return null;
            }
        }

        /**
         * @brief Initialization function for the DisableHP behaviour.
         *
        ...
        {
            Debug.LogInfo("[DisableHP] Initializing for entity {0}", entity.ID);
        }

        /**
         * @brief Update function called every frame for the DisableHP behaviour.
         *
        ...
        {
            var button = GameLogicSystem.Instance.GetBehaviour<DisableHP>(entity);

            if (button == null || button._buttonComponent == null)
            {
                Debug.LogWarning("[DisableHP] ButtonComponent is null for entity {0}.", entity.ID);
                return;
            }
            ...
            try
            {
                if (Input_IsMouseOverEntity(entity.ID))
                {
                    ...
                    ulong HpBarFont = InternalCalls.Entity_GetEntityByName("HealthBar");
                    InternalCalls.Font_SetFont(HpBarFont, "100 I100", 0.0f);
                }
                else
                {
                    button._buttonComponent.IsHovered = false;
                    ulong HpBarFont = InternalCalls.Entity_GetEntityByName("HealthBar");
                    InternalCalls.Font_SetFont(HpBarFont, "100 I100", 0.6f);
                }
            }
            catch (Exception ex)
            {
                Debug.LogError("[DisableHP] Error during update: {0}", ex.Message);
            }
        }

        /**
         * @brief Cleanup function for the DisableHP behaviour.
         *
        ...
        public static void End(Entity entity)
        {
            ...
            Debug.LogInfo("[DisableHP] Cleanup complete for entity {0}.", entity.ID);
        }
        ...
        private static bool Input_IsMouseOverEntity(ulong entityID)
        {
            ...
            {
                Debug.LogError("[DisableHP] Entity {0} missing Transform: {1}", entityID, ex.Message);
                return false;
            }
